...
static inline bool Send_LOW(DALI_t* self) 
{
    if (read_rx(self)==HARDW_LOW) return DALI_COLLISION; //Collesion
    Set_tx_low(self);
...
{
    self->sleep_counter=0;
    if (self->flags.hat_error_flag) return 1;
...
static void timeout_impl(DALI_t* self, TIM_TypeDef *TIMx) {
    ...
    if (TIMx == self->sleepTimer) {
      ...
      //  LL_GPIO_TogglePin(GPIOA, ROLE5_Pin);
        ...
        // --- NVM YAZMA KONTROLÜ ---
        if (self->flags.is_dirty && ++self->dirty_counter > 250) {
            self->flags.save_request_flag = true;
            self->flags.is_dirty = false;
            self->dirty_counter = 0;
        }

        // --- UYUMA SİNYALİ ---
        //if (++self->sleep_counter > 3000) self->flags.sleep_flag = 1;
        return;
    }
    ...
    if (TIMx == self->rxTimer.tim) 
    {
        ...
        if (self->rx_state == STATE_RX) 
        {
            ...
            if (self->Adres!=NULL && bt!=0) {
                ...
                uint8_t received_short_addr = (self->addr_byte >> 1) & 0x3F;
                if (Is_Special(self->addr_byte)) cevaplandi = cevapla02(self);
                if (!cevaplandi && received_short_addr == self->Adres->short_address)
                  {
                      cevaplandi = cevapla01(self); 
                  }
                if (!cevaplandi && self->data_callback) self->data_callback(self->rx_data,bt);          
            } else if (self->error_callback) self->error_callback(9);
        }
        CIK:
        self->rx_state = STATE_IDLE;
        self->bit_idx = 0;
...
void loop_impl(DALI_t* self) {
      if (self->flags.save_request_flag) {
        write_eeprom(self->Adres);
        self->flags.save_request_flag= false;
      }
